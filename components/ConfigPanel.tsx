...
import { Plus, Trash2, Settings2, BarChart2, X, ArrowRightLeft } from 'lucide-react';
import { ChartConfig, YColumnConfig } from '../types';
import { CHART_COLORS, getNextColor } from '../utils/colorUtils';
...
export const ConfigPanel: React.FC<ConfigPanelProps> = ({
  ...
  const addYColumn = () => {
    ...
    const newY: YColumnConfig = {
      ...
      column: columns.find(c => c !== activeChart.xColumn) || columns[0],
      axis: 'left',
      ...
      {activeChart ? (
        <div className="p-4 overflow-y-auto flex-1 space-y-6">
          ...
          <div>
            ...
            <select
              value={activeChart.xColumn}
              onChange={(e) => onUpdateChart(activeChart.id, { xColumn: e.target.value })}
              className="w-full p-2 bg-white border border-slate-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
            >
              {columns.map(col => (
              ...
              {activeChart.yColumns.map((yCol) => (
                <div key={yCol.id} className="p-3 border border-slate-200 rounded-lg bg-slate-50 relative group">
                  ...
                  <div className="mb-3 pr-6">
                    ...
                    <select
                      value={yCol.column}
                      onChange={(e) => updateYColumn(yCol.id, { column: e.target.value })}
                    ...
                    <select
                      value={yCol.xColumn || ''}
                      onChange={(e) => updateYColumn(yCol.id, { xColumn: e.target.value || undefined })}
                      className="w-full p-1.5 bg-slate-50 border border-slate-200 rounded text-xs text-slate-600"
                    >
                      <option value="">Use Default ({activeChart.xColumn})</option>
                      {columns.map(col => (
                        <option key={col} value={col}>{col}</option>
                      ))}
                    </select>
